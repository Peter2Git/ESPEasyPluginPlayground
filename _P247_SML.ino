...
#define P247_DEV_ID          PCONFIG(0)
#define P247_DEV_ID_LABEL    PCONFIG_LABEL(0)
#define P247_MODEL           PCONFIG(1)
#define P247_MODEL_LABEL     PCONFIG_LABEL(1)
#define P247_BAUDRATE        PCONFIG(2)
#define P247_BAUDRATE_LABEL  PCONFIG_LABEL(2)
#define P247_QUERY1          PCONFIG(3)
#define P247_QUERY2          PCONFIG(4)
#define P247_QUERY3          PCONFIG(5)
#define P247_QUERY4          PCONFIG(6)
#define P247_DEPIN           CONFIG_PIN3

#define P247_DEV_ID_DFLT     1
#define P247_MODEL_DFLT      0  // SDM120C
...
#define P247_QUERY1_CONFIG_POS  3
...
ESPeasySerial* Plugin_247_SoftSerial = NULL;

SDM* Plugin_247_SDM = NULL;


boolean Plugin_247_init = false;
...
boolean Plugin_247(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_SET_DEFAULTS:
      {
        P247_DEV_ID = P247_DEV_ID_DFLT;
        P247_MODEL = P247_MODEL_DFLT;
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
        ...
        if (P247_DEV_ID == 0 || P247_DEV_ID > 247 || P247_BAUDRATE >= 6) {
          // Load some defaults
          P247_DEV_ID = P247_DEV_ID_DFLT;
          P247_MODEL = P247_MODEL_DFLT;
          P247_BAUDRATE = P247_BAUDRATE_DFLT;
        ...
        addFormNumericBox(F("Modbus Address"), P247_DEV_ID_LABEL, P247_DEV_ID, 1, 247);

        {
          String options_model[4] = { F("SDM120C"), F("SDM220T"), F("SDM230"), F("SDM630") };
          addFormSelector(F("Model Type"), P247_MODEL_LABEL, 4, options_model, NULL, P247_MODEL );
        }
        {
          ...
          addFormSelector(F("Baud Rate"), P247_BAUDRATE_LABEL, 6, options_baudrate, NULL, P247_BAUDRATE );
        }

        if (P247_MODEL == 0 && P247_BAUDRATE > 3)
          addFormNote(F("<span style=\"color:red\"> SDM120 only allows up to 9600 baud with default 2400!</span>"));

        if (P247_MODEL == 3 && P247_BAUDRATE == 0)
          addFormNote(F("<span style=\"color:red\"> SDM630 only allows 2400 to 38400 baud with default 9600!</span>"));

        if (Plugin_247_SDM != nullptr) {
          addRowLabel(F("Checksum (pass/fail)"));
          String chksumStats;
          chksumStats = Plugin_247_SDM->getSuccCount();
          chksumStats += '/';
          chksumStats += Plugin_247_SDM->getErrCount();
          addHtml(chksumStats);
        }

        {
          ...
          String options[P247_NR_OUTPUT_OPTIONS];
          for (int i = 0; i < P247_NR_OUTPUT_OPTIONS; ++i) {
            options[i] = P247_getQueryString(i);
          }
          for (byte i = 0; i < P247_NR_OUTPUT_VALUES; ++i) {
            const byte pconfigIndex = i + P247_QUERY1_CONFIG_POS;
            sensorTypeHelper_loadOutputSelector(event, pconfigIndex, i, P247_NR_OUTPUT_OPTIONS, options);
          }
        }
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
          ...
          for (byte i = 0; i < P247_NR_OUTPUT_VALUES; ++i) {
            ...
            sensorTypeHelper_saveOutputSelector(event, pconfigIndex, i, P247_getQueryValueString(choice));
          }

          P247_DEV_ID = getFormItemInt(P247_DEV_ID_LABEL);
          P247_MODEL = getFormItemInt(P247_MODEL_LABEL);
    ...
    case PLUGIN_INIT:
      {
        Plugin_247_init = true;
        if (Plugin_247_SoftSerial != NULL) {
          delete Plugin_247_SoftSerial;
          Plugin_247_SoftSerial=NULL;
        }
        Plugin_247_SoftSerial = new ESPeasySerial(CONFIG_PIN1, CONFIG_PIN2);
        unsigned int baudrate = P247_storageValueToBaudrate(P247_BAUDRATE);
        Plugin_247_SoftSerial->begin(baudrate);

        if (Plugin_247_SDM != NULL) {
          delete Plugin_247_SDM;
          Plugin_247_SDM=NULL;
        }
        Plugin_247_SDM = new SDM(*Plugin_247_SoftSerial, baudrate, P247_DEPIN);
        Plugin_247_SDM->begin();
        success = true;
        break;
      }
    ...
    case PLUGIN_READ:
      {
        if (Plugin_247_init)
        {
          int model = P247_MODEL;
          byte dev_id = P247_DEV_ID;
          UserVar[event->BaseVarIndex]     = P247_readVal(P247_QUERY1, dev_id, model);
          UserVar[event->BaseVarIndex + 1] = P247_readVal(P247_QUERY2, dev_id, model);
          UserVar[event->BaseVarIndex + 2] = P247_readVal(P247_QUERY3, dev_id, model);
          UserVar[event->BaseVarIndex + 3] = P247_readVal(P247_QUERY4, dev_id, model);
          success = true;
          break;
        }
        break;
...
float P247_readVal(byte query, byte node, unsigned int model) {
  if (Plugin_247_SDM == NULL) return 0.0;

  byte retry_count = 3;
  bool success = false;
  float _tempvar = NAN;
  while (retry_count > 0 && !success) {
    Plugin_247_SDM->clearErrCode();
    _tempvar = Plugin_247_SDM->readVal(P247_getRegister(query, model), node);
    --retry_count;
    if (Plugin_247_SDM->getErrCode() == SDM_ERR_NO_ERROR) {
      success = true;
    }
  }
...
unsigned int P247_getRegister(byte query, byte model) {
  if (model == 0) { // SDM120C
    switch (query) {
