...
#define P246_NR_OUTPUT_OPTIONS        10
...
ESPeasySerial* Plugin_246_SoftSerial = NULL;

SDM* Plugin_246_SDM = NULL;


boolean Plugin_246_init = false;
...
boolean Plugin_246(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
...

        {
          ...
          addFormSelector(F("Model Type"), P246_MODEL_LABEL, 4, options_model, NULL, P246_MODEL );
        }
        {
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
          ...
          P246_MODEL = getFormItemInt(P246_MODEL_LABEL);
          P246_BAUDRATE = getFormItemInt(P246_BAUDRATE_LABEL);

          Plugin_246_init = false; // Force device setup next time
          success = true;
          break;
      }
    ...
    case PLUGIN_INIT:
      {
        Plugin_246_init = true;
        if (Plugin_246_SoftSerial != NULL) {
          delete Plugin_246_SoftSerial;
          Plugin_246_SoftSerial=NULL;
        }
        Plugin_246_SoftSerial = new ESPeasySerial(CONFIG_PIN1, CONFIG_PIN2);
        unsigned int baudrate = P246_storageValueToBaudrate(P246_BAUDRATE);
        Plugin_246_SoftSerial->begin(baudrate);

        if (Plugin_246_SDM != NULL) {
          delete Plugin_246_SDM;
          Plugin_246_SDM=NULL;
        }
        Plugin_246_SDM = new SDM(*Plugin_246_SoftSerial, baudrate, P246_DEPIN);
        Plugin_246_SDM->begin();
        success = true;
        break;
      }
    ...
    case PLUGIN_READ:
      {
        if (Plugin_246_init)
        {
          int model = P246_MODEL;
          byte dev_id = P246_DEV_ID;
          UserVar[event->BaseVarIndex]     = P246_readVal(P246_QUERY1, dev_id, model);
          UserVar[event->BaseVarIndex + 1] = P246_readVal(P246_QUERY2, dev_id, model);
          UserVar[event->BaseVarIndex + 2] = P246_readVal(P246_QUERY3, dev_id, model);
          UserVar[event->BaseVarIndex + 3] = P246_readVal(P246_QUERY4, dev_id, model);
          success = true;
          break;
        }
        break;
...
float P246_readVal(byte query, byte node, unsigned int model) {
  if (Plugin_246_SDM == NULL) return 0.0;

  byte retry_count = 3;
  bool success = false;
  float _tempvar = NAN;
  while (retry_count > 0 && !success) {
    Plugin_246_SDM->clearErrCode();
    _tempvar = Plugin_246_SDM->readVal(P246_getRegister(query, model), node);
    --retry_count;
    if (Plugin_246_SDM->getErrCode() == SDM_ERR_NO_ERROR) {
      success = true;
    }
  }
...
unsigned int P246_getRegister(byte query, byte model) {
  if (model == 0) { // SDM120C
    switch (query) {
